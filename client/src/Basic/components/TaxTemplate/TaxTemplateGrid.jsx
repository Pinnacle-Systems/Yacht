...
const TaxTemplateGrid = ({
  ...
  const handleDeleteRow = (id) => {
    setTaxTemplateItems((tax) =>
      tax.filter((row, index) => index !== parseInt(id))
    );
  };
  ...
  function findIdInTaxTerms(id) {
    return taxTemplateItems
      ? taxTemplateItems.find(
          (taxItems) => parseInt(taxItems.taxTermId) === parseInt(id)
        )
      : false;
  }
          ...
          <div className={`w-full overflow-y-auto p-3 `}>
            <table className=" border border-gray-600 text-xs table-auto w-full">
              <thead className="bg-gray-100 top-0">
                <tr>
                  <th className="table-data border border-gray-600 w-28">
                    Tax Name
                  </th>
                  <th className="table-data border border-gray-600 w-32">
                    Display Name
                  </th>
                  ...
                  <tr key={index} className="w-full">
                    <td className="flex justify-center items-center">
                      <select
                        ...
                        value={row.taxTermId}
                        onChange={(e) =>
                          handleInputChange(e, index, "taxTermId")
                        }
                      >
                        ...
                        {TaxTermList.data.map((taxItems) => (
                          <option
                            value={taxItems.id}
                            key={taxItems.id}
                            hidden={findIdInTaxTerms(taxItems.id)}
                          >
                            {taxItems.name}
                          </option>
                        ))}
                    ...
                    >
                      {readOnly ? (
                        ...
                        <div
                          ...
                          className="flex justify-center px-2 py-2 items-center rounded bg-gray-300"
                        >
                          {DELETE}
                        </div>
                      )}
