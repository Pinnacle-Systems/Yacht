...
} from "../../../redux/uniformService/ReferenceMasterService";
import { UserPermissions } from "../../../Utils/UserPermissions";

const MODEL = "Reference Master";
...
export default function Form() {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetReferenceMasterQuery({ params, searchParams: searchValue });
  const {
    data: singleData,
    isFetching: isSingleFetching,
    isLoading: isSingleLoading,
  } = useGetReferenceMasterByIdQuery(id, { skip: !id });
  const [trigger, { data: singleDataLazy, isFetchingLazy }] =
    useLazyGetReferenceMasterByIdQuery();
  const [addData] = useAddReferenceMasterMutation();
  const [updateData] = useUpdateReferenceMasterMutation();
  const [removeData] = useDeleteReferenceMasterMutation();
  ...
  useEffect(() => {
    if (form && !readOnly && referenceNameRef.current) {
      referenceNameRef.current.focus();
    }
  }, [form, readOnly]);
  ...
  const saveData = () => {
    ...
    if (foundItem) {
      Swal.fire({
        text: "The Reference Name already exists.",
        icon: "warning",
        timer: 1500,
        showConfirmButton: false,
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      className: "font-medium text-right text-gray-900  w-[50px]  py-1  px-2",
      search: "Reference Name",
    },
    {
      header: "Status",
