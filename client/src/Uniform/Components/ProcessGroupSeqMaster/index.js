...
export default function Form() {
  ...
  const {
    ...
    isLoading,
    isFetching,
  } = useGetProcessGroupSeqMasterQuery({ params, searchParams: searchValue });
  const {
    data: singleData,
    isFetching: isSingleFetching,
    isLoading: isSingleLoading,
  } = useGetProcessGroupSeqMasterByIdQuery(id, { skip: !id });
  ...
  const syncFormWithDb = useCallback(
    (data) => {
      if (!id) {
        ...
        setSequence(data?.sequence ? data?.sequence : "");
      } else {
        setName(data?.name || "");
  ...
  const saveData = () => {
    ...
    if (foundItem) {
      Swal.fire({
        text: "The Process Group Seq Name already exists.",
        icon: "warning",
        timer: 1500,
        showConfirmButton: false,
      });
            ...
            <div className="border-b py-2 px-4 mt-4 mx-3 flex justify-between items-center sticky top-0 z-10 bg-white">
              <div className="flex items-center gap-2">
                <h2 className="text-lg px-2 py-0.5 font-semibold text-gray-800">
                  {id
                    ? !readOnly
                      ? "Edit Process Group Seq  Master"
                      : "Process Group Seq  Master"
                    : "Add New Process Group Seq"}
                </h2>
              </div>
