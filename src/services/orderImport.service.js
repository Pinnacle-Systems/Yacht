...
async function getNextDocId(branchId) {
  ...
  let newDocId = `${branchObj.branchCode}/${shortCode}/ORDI/1`;
  if (lastObject) {
    newDocId = `${branchObj.branchCode}/${shortCode}/ORDI/${parseInt(lastObject.docId.split("/").at(-1)) + 1}`;
  }
  return newDocId;
}

async function ordergetNextDocId(
  branchId,
  shortCode,
  startTime,
  endTime,
...
async function createAdditionalImportData(
  tx,
...
async function createOrderImportItems(additionalImportData) {
  ...
  for (let i = 0; i < additionalImportData?.length; i++) {
    ...
    let bottomColor = await getTableRecordWithId(
      parseInt(obj?.colorId),
      "color",
    );
    ...
    for (let j = 0; j < obj?.classIds?.length; j++) {
      ...
      let sizeName = await getTableRecordWithId(
        parseInt(newObj?.sizeId),
        "size",
      );
      sizeName = sizeName ? sizeName.name : "";
      let bottomSize = await getTableRecordWithId(
        parseInt(newObj?.sizeId),
        "size",
      );
...
async function create(req) {
  const { userId, branchId, partyId, companyId, orderId, finYearId } =
    await req.body;
...
async function createOrder(importdata, finYearId, branchId, userId, companyId) {
  ...
  let manufactureId = await findFromList(
    orderImport?.orderImportItems[0]?.manufacturer_mail_id,
    partyData,
  );
  let vendorIdData = await findFromList(
    orderImport?.orderImportItems[0]?.vendor_mail_id,
    partyData,
  );
  ...
  let orderDetails = orderImport?.orderImportItems?.map((val) => {
    return {
      ...
      supplierCode: val?.season_supplier_code
        ? val?.season_supplier_code
        : null,
    };
  });
