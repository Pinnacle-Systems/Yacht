...
async function getNextDocId(branchId) {
  ...
  let newDocId = `${branchObj.branchCode}/${shortCode}/PRO/1`;
  if (lastObject) {
    newDocId = `${branchObj.branchCode}/${shortCode}/PRO/${parseInt(lastObject.docId.split("/").at(-1)) + 1}`;
  }
  return newDocId;
}
...
function getPriority(planEndDate) {
  ...
  if (planEndDate) {
    if (
      moment(new Date(planEndDate).getTime()).format("YYYY-MM-DD") ===
      moment(todayDate.getTime()).format("YYYY-MM-DD")
    ) {
      return "bg-amber-400";
    } else if (
      moment(new Date(planEndDate).getTime()).format("YYYY-MM-DD") >
...
async function getSingleData(projectId) {
  ...
  return {
    ...
    projectDatas: projectData?.LineItems?.flatMap((val) => val.subLineItems)
      .filter(
        (i) =>
          !i.isCompleted &&
          moment(new Date(i.planEndDate).getTime()).format("YYYY-MM-DD") <=
            moment(todayDate.getTime()).format("YYYY-MM-DD"),
      )
      ?.map((val) => ({
    ...
    tx.lineItems.create({
      data: {
        ...
        planStartDate: value["planStartDate"]
          ? new Date(value["planStartDate"])
          : undefined,
        planEndDate: value["planEndDate"]
          ? new Date(value["planEndDate"])
          : undefined,
        ...
        subLineItems: {
          createMany: {
            data: value.subLineItems.map((temp) => {
              ...
              newItem["responsiblePerson"] = temp["responsiblePerson"]
                ? temp["responsiblePerson"]
                : "";
              ...
              newItem["description"] = temp["description"]
                ? temp["description"]
                : "";
              ((newItem["planStartDate"] = temp["planStartDate"]
                ? new Date(temp["planStartDate"])
                : undefined),
                (newItem["planEndDate"] = temp["planEndDate"]
                  ? new Date(temp["planEndDate"])
                  : undefined),
                (newItem["leadDays"] = temp["leadDays"]
                  ? temp["leadDays"]
                  : ""));
...
async function create(body) {
  ...
  const filteredQuotesId = lineItems.filter((val, index) => {
    return (
      lineItems?.findIndex(
        (item) => parseInt(item?.quoteId) === parseInt(val?.quoteId),
      ) === index
    );
  });
...
async function update(id, body) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const oldLineItemsIds = dataFound.LineItems.map((item) =>
      parseInt(item.id),
    );
    const currentLineItemsIds = lineItems
      .filter((i) => i?.id)
      ?.map((item) => parseInt(item.id));
    const removedLineItems = getRemovedItems(
      oldLineItemsIds,
      currentLineItemsIds,
    );
                      ...
                      ? {
                          data: h.subLineItems?.map((temp) => {
                            ...
                            newItem["name"] = temp["name"]
                              ? temp["name"]
                              : null;
                            newItem["responsiblePerson"] = temp[
                              "responsiblePerson"
                            ]
                              ? temp["responsiblePerson"]
                              : null;
                            newItem["isCompleted"] = temp["isCompleted"]
                              ? temp["isCompleted"]
                              : false;
                            newItem["category"] = temp["category"]
                              ? temp["category"]
                              : null;
                            newItem["description"] = temp["description"]
                              ? temp["description"]
