...
import {
  createproductionReceiptByAuto,
  getTableRecordWithId,
} from "../utils/helperQueries.js";
import {
  getDateFromDateTime,
  getYearShortCode,
...
async function getNextDocId(branchId, shortCode, startTime, endTime) {
  ...
  let newDocId = `${branchObj.branchCode}/${shortCode}/DIS/1`;
  if (lastObject) {
    newDocId = `${branchObj.branchCode}/${shortCode}/DIS/${parseInt(lastObject.docId.split("/").at(-1)) + 1}`;
  }
  return newDocId;
}
...
async function createDispatchedDetails(
  tx,
  dispatchedDetails,
  dispatched,
  orderId,
      ...
      data: {
        ...
        stockForPanels: {
          create: {
            ...
            colorId: item.colorId ? parseInt(item.colorId) : undefined,
            panelColorId: item.panelColorId
              ? parseInt(item.panelColorId)
              : undefined,
...
async function updateDispatchedDetails(
  tx,
  dispatchedDetails,
  dispatched,
  orderId,
